...
public class GaloisTest {
	Galois galois = new Galois(29);
	
	
	@Test
	public void addSub(){
		byte add = galois.add((byte) 5, (byte) 3);
		byte sub = galois.sub((byte) 5, (byte) 3);
	...
	public void checkLogTable() {
		short [] generatedLog = galois.getLogTable(); 
		short [] testLog = new short[] {-1,    0,    1,   25,    2,   50,   26,  198,
		...
		for(int i = 0; i < generatedLog.length; i++) {
			assertEquals(generatedLog[i],testLog[i]);
		}
	
	}
	
	@Test
	public void checkExpTable(){
		byte [] generatedExp = galois.getExpTable();
		byte [] testExp = {1,    2,    4,    8,   16,   32,   64, -128,
		...
		for(int i = 0; i < generatedExp.length; i++) {
			//System.out.println(generatedExp[i]);
			assertEquals(generatedExp[i],testExp[i]);
		}
	}
	
	@Test
	public void tableRelationship() {
		short[] logTable = galois.getLogTable();
		byte[] expTable = galois.getExpTable();
		int verify = 20;
		int result1 = logTable[expTable[verify & 0xFF] & 0xFF];
		int result2 = expTable[logTable[verify & 0xFF] & 0xFF];
